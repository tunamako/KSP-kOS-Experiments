...
function readHorizAcc {
  if ( time:seconds - lastHorizAccMeasureTime > 1 ) {
    if ( lastHorizAccMeasureTime > 0 ) {
      set cachedHorizAcc to (ship:groundSpeed - lastShipHorizontalSpeed) /
...
// calculate mean
local avgDrag is 0.
local valuesDrag is list().
local valuesAvgCount is 20. /// max list size for sliding avg
...
function getDrag {
  ...
  valuesDrag:add( drag ).
  if valuesDrag:length > valuesAvgCount {
    valuesDrag:remove(0). // oldest
  }

  local newDrag to 0.
  for c in valuesDrag {
    set newDrag to newDrag + c.
  }

  set avgDrag to newDrag / valuesDrag:length.
...
function getTermVel {
  ...
  local _termVel is 2*sqrt(ship:verticalSpeed^2 + ship:groundSpeed^2).
    //todo drag
  if body:atm:exists {
